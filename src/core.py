...
# matplotlib imported lazily only when show_image_level >= 1 (see blocks below)
    ...
    def read_omr_response(self, template, image, name, save_dir=None, evaluation_config=None):
        ...
        try:
                    ...
                    for bubble in field_block_bubbles:
                        ...
                        if bubble_is_marked:
                            ...
                            skip_draw = (
                                ...
                                and bubble.field_label in evaluation_config.questions_in_order
                            )
                            if not skip_draw:
                                cv2.rectangle(
                                    final_marked,
                            ...
                            if evaluation_config is None:
                                ...
                                cv2.rectangle(
                                    ...
                                    (
                                        int(x_unmarked + box_w - box_w / 10),
                                        int(y_unmarked + box_h - box_h / 10),
                                    ),
                                    CLR_GRAY,
                                    -1,
                                )
            ...
            if evaluation_config is not None:
                ...
                transp_layer = cv2.cvtColor(transp_layer, cv2.COLOR_GRAY2BGR)
                empty_val = template.global_empty_val or ""
                circles_drawn = {"correct": 0, "incorrect": 0, "unmarked": 0}
                for field_block in template.field_blocks:
                    box_w, box_h = field_block.bubble_dimensions
                    shift = field_block.shift
                    for field_block_bubbles in field_block.traverse_bubbles:
                        for bubble in field_block_bubbles:
                            if bubble.field_label not in evaluation_config.questions_in_order:
                                continue
                            marked = omr_response.get(bubble.field_label, empty_val)
                            if bubble.field_value != marked:
                                continue
                            verdict = evaluation_config.get_verdict_for_question(
                                bubble.field_label, marked
                            )
                            if verdict.startswith("correct"):
                                color = (0, 255, 0)  # BGR green
                                circles_drawn["correct"] += 1
                            elif verdict == "incorrect":
                                color = (0, 0, 255)  # BGR red
                                circles_drawn["incorrect"] += 1
                            else:
                            ...
                            if overlay_cfg is not None and hasattr(overlay_cfg, "toDict"):
                                overlay_cfg = overlay_cfg.toDict()
                            overlay_cfg = overlay_cfg or {}
                            radius_ratio = overlay_cfg.get("circle_radius_ratio", 0.85)
                            thickness = overlay_cfg.get("circle_thickness", 3)
                            radius = max(1, int(half * radius_ratio))
                            cv2.circle(final_marked, center, radius, color, thickness)
                total_circles = sum(circles_drawn.values())
                logger.info(
                    "Evaluation overlay: drew %d circles (correct=%d, incorrect=%d, unmarked=%d) for %s"
                    % (
                        total_circles,
                        circles_drawn["correct"],
                        circles_drawn["incorrect"],
                        circles_drawn["unmarked"],
                        name,
                    )
                )
            else:
                logger.info("Evaluation overlay: skipped (no evaluation_config) for %s" % name)
    ...
    def get_local_threshold(
        ...
        if len(q_vals) < 3:
            ...
            if max1 < confident_jump:
                if no_outliers:
                    # Strip has no clear filled/empty split (e.g. all empty). Use stricter
                    # threshold so only clearly dark bubbles count; avoids false positives
                    # on blank sheets (shadows, paper texture).
                    strict = config.threshold_params.get("STRICT_EMPTY_THRESHOLD", 140)
                    thr1 = min(global_thr, strict)
                else:
