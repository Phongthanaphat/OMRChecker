...
class EvaluationConfig:
    ...
    def prepare_and_validate_omr_response(self, omr_response):
        ...
        if len(missing_prefixed_questions) > 0:
            logger.warning(
                f"Questions in OMR response not in answer key (will not be graded): {missing_prefixed_questions}"
            )
    ...
    def conditionally_save_explanation_csv(self, file_path, evaluation_output_dir):
        if self.enable_evaluation_table_to_csv and self.explanation_table is not None:
    ...
    def get_verdict_for_question(self, question, marked_answer):
        """Return verdict string: 'correct', 'incorrect', 'unmarked', or 'correct-X' for multiple-correct."""
        if question not in self.question_to_answer_matcher:
            return "unmarked"
        answer_matcher = self.question_to_answer_matcher[question]
        question_verdict, _ = answer_matcher.get_verdict_marking(marked_answer)
        return question_verdict
    ...
    def conditionally_add_explanation(
        ...
        if self.should_explain_scoring:
            ...
            self.explanation_table.add_row(*row)  # pyright: ignore[reportOptionalMemberAccess]
