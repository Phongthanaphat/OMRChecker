...
### เก็บภาพที่ประมวลผลไม่ผ่านชั่วคราว

ใช้เฉพาะช่วงวิเคราะห์ภาพจริง โดยเพิ่ม environment ของ service:

```bash
OMR_DEBUG_SAVE_FAILED_IMAGES=1
OMR_DEBUG_FAILED_IMAGE_MAX=20
```

ภาพจาก request ที่ตอบ `400` จะอยู่ที่ `outputs/debug/failed/` และระบบลบไฟล์เก่าสุด
อัตโนมัติเมื่อเกินจำนวนที่กำหนด (จำกัดสูงสุด 100 ภาพ) โฟลเดอร์นี้ไม่มี endpoint
สำหรับเปิดจากเว็บและอาจมีข้อมูลนักเรียน จึงควรปิดค่าดังกล่าวหลังเก็บตัวอย่างครบแล้ว
