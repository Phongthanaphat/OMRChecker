...
- **เรียก OMR API จาก Laravel (server-side)** ยังใช้ `http://127.0.0.1:8080` ได้ตามเดิม  
  - เช่น `POST http://127.0.0.1:8080/check` — **ต้องส่ง `exam_id` ทุกครั้ง** (บังคับใน API) ไม่ส่งจะได้ HTTP 400  
  - Laravel รันบน VPS เดียวกับ OMR API ดังนั้น 127.0.0.1:8080 ใช้ได้จาก Laravel
- **`POST /check` อาจคืน HTTP 400 เมื่อ Roll ไม่ผ่านการตรวจ** (เทมเพลตที่มี `customLabels.Roll` เช่น `50q`) — ไม่มี `responses` / รูป checked ใน response สำเร็จ; ให้แสดง `detail` จาก API แก่ผู้ใช้ (มักให้สแกนใหม่ภาพชัดขึ้น) — รายละเอียดใน `docs/API.md` หัวข้อ "การตรวจ Roll"
...
- ตั้ง **Nginx** ให้ proxy path หนึ่ง (เช่น `/api/omr/`) ไปที่ `http://127.0.0.1:8080/`

  ```nginx
  # ต้องอยู่ก่อน location / (Laravel)
  location /api/omr/ {
...
- **ฝั่ง Laravel:** ตอนสร้าง URL รูป “ดูกระดาษคำตอบ” ใช้ **URL ผ่าน domain** แทน 127.0.0.1:8080  
  - ตัวอย่าง: `https://yourdomain.com/api/omr/checked/school/12/42/by-roll/12345.jpg` (หรือรูปแบบเดิมที่มี `YYYY-MM/` เมื่อเทมเพลตไม่มี Roll)  
  - หรือใช้ `url('/api/omr/checked/' . $checked_omr_filename)` โดยใช้ค่า `checked_omr_filename` จาก response ล่าสุดเสมอ
- ผล: ผู้ใช้กดลิงก์ → browser เรียก yourdomain.com → Nginx ส่งต่อไป OMR API ที่ 127.0.0.1:8080 → ได้รูป

> ⚠️ **ตอนนี้ทุก endpoint ต้องมี `Authorization: Bearer <key>`** → browser โหลดรูปตรง ๆ ผ่าน `/api/omr/checked/...` จะ 401 — ต้องใช้ **แบบ B (Laravel proxy)** หรือ whitelist `/checked/` ออกจาก auth (ดู `docs/API.md` หัวข้อ "ผลกระทบ: การแสดงรูป Checked OMR")

### แบบ B: ให้ Laravel เป็นคนส่งรูปให้ browser (proxy ผ่าน Laravel)

- สร้าง **route ใน Laravel** ที่รับ path รูป (เช่น `school/12/42/by-roll/12345.jpg` หรือ `school/12/42/2026-05/uuid_stem.jpg`)  
...
- ลิงก์ “ดูกระดาษคำตอบ” ชี้ไปที่ **route นี้**  
  - ตัวอย่าง: `https://yourdomain.com/omr/checked-image/school/12/42/by-roll/12345.jpg`
