...
- ถ้า `require_roll=true` แล้ว Roll ไม่ผ่าน (เช่น อ่านได้ `33` หรือ `13424235454` มักเพราะภาพไม่ชัดหรือกรอบไม่ครบ) API จะยังตรวจคะแนนต่อและตอบ **HTTP 200** พร้อม `warnings[].code = "invalid_roll"` เพื่อให้ Laravel บันทึกเป็นรายการรอแก้รหัสนักเรียน
- รูป checked ของเคส Roll ไม่ผ่านจะเก็บแบบ `<YYYY-MM>/<uuid>_<ชื่อไฟล์>.jpg` ไม่ใช้ `by-roll/{roll}.jpg` เพื่อป้องกันการเขียนทับไฟล์ผิดคน
