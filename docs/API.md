...
- **Response:** JSON
  ...
  - `warnings` (ถ้ามี): รายการคำเตือนแบบไม่ fatal เช่น `incomplete_roll` เมื่อโหมด student อ่านรหัสนักเรียนได้ 2-4 หลัก จึงตรวจคะแนนได้แต่ควรให้ Laravel บันทึกแบบยังไม่ผูกนักเรียน
...
- ต้องเป็น **ตัวเลข 0–9 ล้วน** และความยาว **2 ถึงจำนวนช่อง roll** ในเทมเพลต (เช่น 50q มี 5 ช่อง → รับเฉพาะ 2-5 หลัก)
- ถ้า `require_roll=true` แล้ว Roll ครบจำนวนช่องของเทมเพลต (เช่น 5 หลัก) API จะตอบ **HTTP 200** โดยไม่มี Roll warning เพื่อให้ Laravel auto-bind นักเรียนได้ถ้าพบรหัสในฐานข้อมูล
- ถ้า `require_roll=true` แล้ว Roll เป็นตัวเลขแต่ยังไม่ครบจำนวนช่องของเทมเพลต (เช่น `33` หรือ `0844`) API จะยังตรวจคะแนนต่อและตอบ **HTTP 200** พร้อม `warnings[].code = "incomplete_roll"` เพื่อให้ Laravel บันทึกเป็นรายการรอแก้รหัสนักเรียน และไม่ผูกนักเรียนอัตโนมัติ
- ถ้า Roll ว่าง, อ่านได้ 1 หลัก, ยาวเกินจำนวนช่องของเทมเพลต, หรือมีอักขระผิดรูปแบบ (เช่น `13424235454` หรือ `12AB`) API จะตอบ **HTTP 400** เพราะถือว่าภาพ/การกรอกรหัสไม่น่าเชื่อถือ ต้องสแกนใหม่
- รูป checked ของเคส Roll 2-4 หลักจะเก็บแบบ `<YYYY-MM>/<uuid>_<ชื่อไฟล์>.jpg` ไม่ใช้ `by-roll/{roll}.jpg` เพื่อป้องกันการเขียนทับไฟล์ผิดคน
