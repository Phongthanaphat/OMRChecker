...
- **Response:** JSON
  ...
  - `warnings` (ถ้ามี): รายการคำเตือนแบบไม่ fatal เช่น `invalid_roll` เมื่อโหมด student อ่านรหัสนักเรียนไม่ได้/ไม่ครบ แต่ยังตรวจคะแนนได้
...
- ถ้า `require_roll=true` แล้ว Roll ว่างหรืออ่านได้สั้นกว่าจำนวนช่องของเทมเพลต (เช่น template 5 ช่อง แต่อ่านได้ `0844`) API จะยังตรวจคะแนนต่อและตอบ **HTTP 200** พร้อม `warnings[].code = "incomplete_roll"` หรือ `invalid_roll` เพื่อให้ Laravel บันทึกเป็นรายการรอแก้รหัสนักเรียน และไม่ผูกนักเรียนอัตโนมัติจากรหัสที่อ่านไม่ครบ
- ถ้า Roll อ่านได้ยาวเกินจำนวนช่องของเทมเพลตหรือมีอักขระผิดรูปแบบ (เช่น template มี 5 ช่องแต่อ่านได้ `13424235454`) API จะตอบ **HTTP 400** เพราะถือว่าภาพ/การ align ไม่น่าเชื่อถือ และคำตอบอาจถูกอ่านผิดตามไปด้วย
- รูป checked ของเคส Roll ว่าง/สั้นจะเก็บแบบ `<YYYY-MM>/<uuid>_<ชื่อไฟล์>.jpg` ไม่ใช้ `by-roll/{roll}.jpg` เพื่อป้องกันการเขียนทับไฟล์ผิดคน
